...
# def test_sample_wo_pattern_autoregressive():
#     model_id = "sshleifer/tiny-gpt2"
#     promptzl, device = init_promptzl(model_id, True)
...
#         output = promptzl.forward(batch, combine=False)
#         assert output.shape[0] == len(batch["input_ids"]) and output.shape[1] == 4
#         pytest.approx(len(batch), torch.sum(output), abs=0.1)

# def test_sample_wo_pattern_mlm():
#     model_id = "nreimers/BERT-Tiny_L-2_H-128_A-2"
#     promptzl, device = init_promptzl(model_id, False)
