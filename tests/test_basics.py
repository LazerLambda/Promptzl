...
class TestPromptzel:
    ...
    def test_sample_wo_pattern(self):
        test = promptzel.LLM4ForPatternExploitationClassification("distilbert/distilgpt2", verbalizer=[["bad"], ["good"]])
        reviews = [
        ...
        test.tokenizer.pad_token_id = test.tokenizer.convert_tokens_to_ids(test.tokenizer.pad_token)

        # Tokenize the dataset
        def tokenize_function(examples):
            return test.tokenizer(list(map(lambda e: e[0] + e[1], zip(examples["text"], [" Overall, it was "]*len(examples["text"])))), 
                            padding="max_length", 
                            truncation=True,
                            return_tensors="pt")
